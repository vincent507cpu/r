#This script is for a project titled "PROJECT: VISUALIZING INEQUALITIES IN LIFE EXPECTANCY" on DataCamp.
...
# Subseting data to obtain countries of interest
top <- subdata2 %>% arrange(desc(diff_Male+diff_Female)) %>% head(3)
bottom <- subdata2 %>% arrange(desc(diff_Male+diff_Female)) %>% tail(3)
